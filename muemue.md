
## Workflow/RAG

- LLMで業務ワークフローを自動生成・最適化する! 〜ワークフロー自動生成・最適化の取り組みについて〜
  - https://tech.layerx.co.jp/entry/2025/11/19/133143

- LM Studio rag-v1
  - https://lmstudio.ai/lmstudio/rag-v1

- 【コピペでOK】Gemini RAGはこんなに簡単！寝てる間にGitHub Actionsが最新ドキュメントを学習するAIアシスタントを作ろう
  - https://qiita.com/sinzy0925/items/01550618a78428c9cc50

- RAGを活用してセキュリティチェックシートの記入作業を自動化した話
  - https://zenn.dev/delisit/articles/0b4977d79500fa


## MCP

- 自然言語でワークフロー作成？Claude Code、n8n、n8n-mcpを組み合わせた開発体験レポート
  - https://zenn.dev/xtm_blog/articles/d8085c176f0040

- 面倒な仕事はMCPにやらせよう 〜AIはどこまで仕事ができるのか？〜
  - https://blog.usize-tech.com/ai-mcp-potentials/

- さくらのクラウドMCPの開発
  - https://knowledge.sakura.ad.jp/45883/

- Claude CodeでBacklogのチケットから自動実装する仕組みが快適すぎた
  - https://qiita.com/mu7kata/items/4b42054ee1b3a96d6f9f

### Serena

- なぜSerenaからLaravel Boostに変えるとAIの精度が上がるのか
  - https://zenn.dev/aun_phonogram/articles/a602649ff478cf

- 話題のMCPサーバー「Serena」をClaude Codeで使ってみた
  - https://zenn.dev/mixi/articles/4b77baf024d8fc

- Claude Code を Serena MCP で強化しよう！Cursorとも連携するよ。
  - https://zenn.dev/ficilcom/articles/da858df304b588

- コーディングエージェントの能力を拡張する Serena を試してみた
  - https://azukiazusa.dev/blog/serena-coding-agent/
  
- なぜSerenaからLaravel Boostに変えるとAIの精度が上がるのか
  - https://zenn.dev/aun_phonogram/articles/a602649ff478cf

## 開発環境

- uv × DockerでのPython開発環境構築方法
  - https://zenn.dev/mkj/articles/3aaa36d6f35c08

- Gemini CLI extensions の概要
  - https://note.com/npaka/n/n509319b6acc6


## 読みもの

- 各AIモデルに「自分のデータセンターを犠牲に人命を助けるか？」と質問した結果とは？ - GIGAZINE
  - https://gigazine.net/news/20251121-ai-sacrifice/

- 検閲を解除した脱獄版LLMを簡単に生成できるツール「Heretic」 - GIGAZINE
  - https://gigazine.net/news/20251117-heretic/
  - https://github.com/p-e-w/heretic

## その他

- Geminiでブラウザを自動操作！Computer Use Previewの使い方
  - https://zenn.dev/emp_tech_blog/articles/gemini-computer-use-preview-usage

- AIエージェントを開発するPdMがやることをプロンプトを書きながら考える
  - https://zenn.dev/layerx/articles/671bdaf4aa0a32

- MCP ツールのコンテキスト圧迫の問題とその解決策
  - https://azukiazusa.dev/blog/mcp-tool-context-overflow/
